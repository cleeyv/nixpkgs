...
in
{
  options.services.vosk = with lib; {
    enable = mkEnableOption "the Vosk WebSocket speech recognition server";
    package = mkOption {
      type = types.package;
      default = pkgs.vosk-server;
      defaultText = literalExpression "pkgs.vosk-server";
      description = "The package to use for the Vosk server.";
    };
    interface = mkOption {
      type = types.str;
      default = "0.0.0.0";
      description = "Interface the Vosk server should bind to.";
    };
    port = mkOption {
      type = types.port;
      default = 2700;
      description = "Port the Vosk server should listen on.";
    };
    model = mkOption {
      type = types.path;
      ...
      description = "Model the Vosk server should use.";
    };
    spkModel = mkOption {
      type = types.nullOr types.path;
      default = pkgs.vosk.models.spk;
      defaultText = literalExpression "pkgs.vosk.models.spk";
      description = "Speaker identification model the Vosk server should use.";
    };
    sampleRate = mkOption {
      type = types.int;
      default = 8000;
      description = "Sample rate the Vosk server should use.";
    };
    maxAlternatives = mkOption {
      type = types.int;
      default = 0;
      description = "The number of alternative guesses the Vosk server should return.";
    };
    showWords = mkOption {
      type = types.bool;
      default = true;
      description = "Whether the Vosk server should return timestamps for recognised words.";
    };
  };
